...
void prehandle::makeMask(cv::Mat depthImage,cv::Mat erodeBinary,int threshold,int erodeTimes,int* classify,Point* matchLocation)
{
	cv::Mat src;
	...
	//redraw the field
	cv::Mat binary(src.size(),CV_8UC1,Scalar(255));
	drawContours(binary, linecontours, -1, Scalar(0), -1);//fill the field
	//make silk mask
	if(*classify==0)//recycle
	{
		rectangle(binary, matchLocation[0],matchLocation[1],Scalar(0,0,255), CV_FILLED, 8, 0 );
		rectangle(binary,Point(matchLocation[0].x+30,matchLocation[0].y-660),\
						 Point(matchLocation[0].x+75,matchLocation[0].y-605),Scalar(0,0,255), CV_FILLED, 8, 0);//apple

		rectangle(binary,Point(matchLocation[0].x,matchLocation[0].y+88),\
						 Point(matchLocation[0].x+155,matchLocation[0].y+155),Scalar(0,0,255), CV_FILLED, 8, 0);//line

		rectangle(binary,Point(matchLocation[0].x-10,matchLocation[0].y-550),\
						 Point(matchLocation[0].x+115,matchLocation[0].y-500),Scalar(0,0,255), CV_FILLED, 8, 0);//words

		rectangle(binary,Point(matchLocation[0].x+20,matchLocation[0].y-100),\
						 Point(matchLocation[0].x+75,matchLocation[0].y-40),Scalar(0,0,255), CV_FILLED, 8, 0);//logo

		rectangle(binary,Point(0,0),Point(depthImage.cols,40),Scalar(0,0,255), CV_FILLED, 8, 0);//header

		rectangle(binary,Point(0,matchLocation[0].y+160),\
	...
	if(*classify==2)//apple
	{
		...
		rectangle(binary,Point(matchLocation[0].x,matchLocation[0].y+740),\
						 Point(matchLocation[0].x+400,matchLocation[0].y),Scalar(0,0,255), CV_FILLED, 8, 0);//line
		rectangle(binary,Point(0,0),Point(depthImage.cols,40),Scalar(0,0,255), CV_FILLED, 8, 0);//header

		rectangle(binary,Point(0,matchLocation[0].y+800),\
						 Point(depthImage.cols,depthImage.rows),Scalar(0,0,255), CV_FILLED, 8, 0);//bottom
	}
	if(*classify==3)//no
	{
		rectangle(binary,Point(0,0),Point(depthImage.cols,40),Scalar(0,0,255), CV_FILLED, 8, 0);//header
		rectangle(binary,Point(0,depthImage.rows-50),\
						 Point(depthImage.cols,depthImage.rows),Scalar(0,0,255), CV_FILLED, 8, 0);//bottom
	}
...
{
	string path_recycle="D:\\VS_Project\\ScanInterface\\bin\\x64\\model\\model_recycle.png";
	string path_error="D:\\VS_Project\\ScanInterface\\bin\\x64\\model\\model_error.png";
	string path_apple="D:\\VS_Project\\ScanInterface\\bin\\x64\\model\\model_apple.png";
...
int prehandle::prejudge(cv::Mat depthImage)
{
	...
	Mat filterImage,canny,erodeImg,maskErode,blackMask;
	depthImage.copyTo(blackMask);
	makeMask(depthImage,blackMask,5,5,classify,matchLocation);
	//imshow("blackMask",blackMask);

	boxFilter(depthImage, filterImage, -1, Size(4,4));
	//imshow("filter",filterImage);
	Canny( filterImage, canny, 30, 40,3);  
